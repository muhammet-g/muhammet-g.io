...
document.querySelectorAll("select[name = 'country']").forEach((item) => {
  item.addEventListener("change", () => {
    ...
    const cities = arabCountriesData[country];
    document
      .querySelectorAll("#paymentcities option")
      .forEach((option) => option.remove());

    const firstOptin = document.createElement("option");
    const optionText = document.createTextNode("اختر المدينة");
    firstOptin.appendChild(optionText);
    firstOptin.setAttribute("value", "");
    firstOptin.setAttribute("disabled", "true");
    firstOptin.setAttribute("selected", "true");

    const city_option = document.getElementById("paymentcities");
    city_option.appendChild(firstOptin);

    cities.forEach((city) => {
      const newOption = document.createElement("option");
      const optionText = document.createTextNode(city);
      newOption.appendChild(optionText);
      firstOptin.setAttribute("value", city);
